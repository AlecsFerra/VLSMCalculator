...
public class VLSMCalculator {
    ...
    public static int[] getNextAddress(int add[]) {
        ...
        int[] nextAddress = Arrays.copyOfRange(add, 0, add.length);
    ...
    public static int[] addHostsToIp(int[] ip, long hosts) {
        int[] nextAddress = Arrays.copyOfRange(ip, 0, ip.length);
    ...
    /**
     * Resets hosts for subnets with balanced mode based on number of hosts
     * available [stored in subnet member variable]
     *
     ...
     *
     * @return VLSMreport, which includes subnet name, network addresse, hosts
     * available, range & broadcast for each subnet
     */
 ...
 /* Initial set up  */
    ...
    /**
     * Sets new network id, validates the format, retrieves CIDR (mask) and
     * tokens
     *
